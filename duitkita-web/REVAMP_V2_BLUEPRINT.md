...
|-----------------|-------|
| [`FRONTEND_CONCEPT.md`](./FRONTEND_CONCEPT.md) | Visual, UX, halaman, komponen — **tetap acuan desain** |
...
| Area | Gap |
|------|-----|
| Response envelope | Sebagian service masih `res.data` langsung; perlu `unwrapApiData()` di **semua** service |
| Refresh token & sessions | API ada, UI/logic belum |
...
| # | Task | API / teknik | Deliverable FE | Prioritas |
|---|------|--------------|----------------|-----------|
| F1 | Envelope di semua service | `unwrapApiData` | Refactor `auth`, `budget`, `expense`, `report`, `activity`, `profile`, `dashboard` | **P0** |
| F2 | Types V2 | DTO backend | Extend `src/types/index.ts`: export job, notification, recurring, reminder, forecast, health, invitation, session | **P0** |
| F3 | `API_ROUTES` lengkap | — | Tambah rute di `src/lib/constants.ts` (lihat §8) | **P0** |
| F4 | Refresh token flow | `POST /auth/refresh` | Simpan refresh; interceptor 401 → refresh sekali → retry; logout jika gagal | **P0** |
| F5 | Error UX | envelope `requestId` | Toast + halaman error tampilkan `requestId` untuk lapor bug | **P1** |
...
*Living document — update setelah setiap sprint FE selesai.*
